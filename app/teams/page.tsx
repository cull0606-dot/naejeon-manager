                      ...
                      return (
                        <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: "var(--surface2)", borderRadius: 8 }}>
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 12, color: "var(--text2)" }}>{p.line} · {p.tier} · {p.wr}% ({p.wins ?? 0}승 {p.losses ?? 0}패)</div>
                          </div>
                          <div style={{ display:"flex", gap:2 }}>
                            {recent.slice(0,5).map((r,i) => (
                              <span key={i} style={{ width:16, height:16, borderRadius:3, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:"#fff", background: r==="W"?"#22C55E":"#EF4444" }}>{r}</span>
                            ))}
                          </div>
